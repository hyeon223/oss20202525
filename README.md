...
|---|---|
|OptionString이 :(콜론)으로 시작되지 않는 경우|Name은 "?" 문자로 설정, OPTARG가 설정되지 않음, 진단 메시지가 표준 오류에 기록됨( getopts명령을 처리중 생긴 오류가 아닌 호출 애플리케이션에 인수가 표시되는 중에 발견된 오류로 간주,진단 메시지는 명시된 대로 기록되지만 종료 상태는 0)|
|OptionString이 :(콜론)으로 시작되는 경우|Name은 "?"로 설정되거나 누락된 필수 옵션에 대해서는 ":" 문자로 설정, OPTARG는 발견된 옵션 문자로 설정, 출력이 표준 오류에 기록되지 않음|

- 옵션의 끝에 도달하는 경우

getopts명령은 0보다 큰 리턴 값으로 종료됨

OPTARG는 첫 번째 비옵션-인수의 색인으로 설정됨 

첫 번째 --인수가 그 전에 다른 비옵션-인수가 나타나지 않는 경우에는 옵션-인수로 간주되고, 비옵션-인수가 없는 경우에는 값 $#+1로 간주됨

Name은 "?" 문자로 설정

옵션의 끝 식별 :  특수 옵션--, -또는 +로 시작되지 않는 인수 찾는 경우, 에러가 발생한 경우
